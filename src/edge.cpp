...
std::string Edge::toString() const {
    ...
    result += "d:" + dst->getName() + " cost=" + std::to_string(getCost()) +
              " userIntField:" + std::to_string(userIntField);
...
class EdgeImpl : public Edge {
 public:
  EdgeImpl(std::shared_ptr<Obj> vertex_src, std::shared_ptr<Obj> vertex_dst,
           std::string const &name, bool directed, int cost)
      : Edge(vertex_src, vertex_dst, name, directed, cost) {}
};

std::shared_ptr<Edge> createEdge(std::shared_ptr<Obj> vertex_src,
                                 std::shared_ptr<Obj> vertex_dst,
                                 std::string const& name, bool directed, int cost) {
    auto edge = std::make_shared<EdgeImpl>(vertex_src, vertex_dst, name, directed, cost);
